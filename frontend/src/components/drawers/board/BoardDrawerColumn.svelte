<script lang="ts">
  ...
  export let emojiKey: number

  const dispatch = createEventDispatcher<{ 'delete-column': number }>()

  const deleteColumn = () => {
    dispatch('delete-column', emojiKey)
  }
</script>
...
>
  ...
  <input
    type="text"
    id={`board-column-${emojiKey}`}
    bind:value={name}
    class="text-base w-full font-medium h-9 px-2 border border-tgray-200 rounded-lg"
  />
  <button
    type="button"
